...
interface IHypercustos is IRageQuit {
    ...
    event HypercustosCreated(address rewardPool, address powerSwitch);
    event HypercustosFunded(uint256 amount, uint256 duration);
    ...
    function getHypercustosData() external view returns (HypercustosData memory hypercustos);
...
/// - Proxy owner:
///     Can change arbitrary logic / state by upgrading the Hypercustos
...
contract Hypercustos is IHypercustos, Powered, Ownable {
    ...
    HypercustosData private _hypercustos;
    ...
    /// @param stakingToken address The address of the staking token for this Hypercustos
    /// @param rewardToken address The address of the reward token for this Hypercustos
    ...
    constructor(
        ...
        require(rewardScaling.floor <= rewardScaling.ceiling, "Hypercustos: floor above ceiling");
        ...
        require(rewardScaling.time != 0, "Hypercustos: scaling time cannot be zero");
        ...
        // commit to storage
        _hypercustos.stakingToken = stakingToken;
        _hypercustos.rewardToken = rewardToken;
        _hypercustos.rewardPool = rewardPool;
        _hypercustos.rewardScaling = rewardScaling;

        // emit event
        emit HypercustosCreated(rewardPool, powerSwitch);
    }
    ...
    function isValidAddress(address target) public view override returns (bool validity) {
        ...
        return
            target != address(this) &&
            target != address(0) &&
            target != _hypercustos.stakingToken &&
            target != _hypercustos.rewardToken &&
            target != _hypercustos.rewardPool &&
            !_bonusTokenSet.contains(target);
    }

    /* Hypercustos getters */

    function getHypercustosData()
        external
        view
        override
        returns (HypercustosData memory hypercustos)
    {
        return _hypercustos;
    }
        ...
        returns (uint256 unlockedRewards)
    {
        ...
        uint256 remainingRewards =
            IERC20(_hypercustos.rewardToken).balanceOf(_hypercustos.rewardPool);
        // calculate reward available based on state
        unlockedRewards = calculateUnlockedRewards(
            _hypercustos.rewardSchedules,
            remainingRewards,
            _hypercustos.rewardSharesOutstanding,
            timestamp
        );
        // explicit return
        ...
        returns (uint256 totalStakeUnits)
    {
        ...
        if (timestamp == _hypercustos.lastUpdate) return _hypercustos.totalStakeUnits;
        // calculate new stake units
        uint256 newStakeUnits =
            calculateStakeUnits(_hypercustos.totalStake, _hypercustos.lastUpdate, timestamp);
        // add to cached total
        totalStakeUnits = _hypercustos.totalStakeUnits.add(newStakeUnits);
        // explicit return
    ...
    /// @notice Add funds to the Hypercustos
    /// access control: only admin
    /// state machine:
    ///   - can be called multiple times
    ///   - only online
    /// state scope:
    ///   - increase _hypercustos.rewardSharesOutstanding
    ///   - append to _hypercustos.rewardSchedules
    ...
    function fund(uint256 amount, uint256 duration) external onlyOwner onlyOnline {
        // validate duration
        require(duration != 0, "Hypercustos: invalid duration");

        // create new reward shares
        // if existing rewards on this Hypercustos
        ...
        if (_hypercustos.rewardSharesOutstanding > 0) {
            uint256 remainingRewards =
                IERC20(_hypercustos.rewardToken).balanceOf(_hypercustos.rewardPool);
            newRewardShares = _hypercustos.rewardSharesOutstanding.mul(amount).div(
                remainingRewards
            );
        } else {
        ...
        _hypercustos.rewardSharesOutstanding = _hypercustos.rewardSharesOutstanding.add(
            newRewardShares
        );

        // store new reward schedule
        _hypercustos.rewardSchedules.push(
            RewardSchedule(duration, block.timestamp, newRewardShares)
        );

        // transfer reward tokens to reward pool
        TransferHelper.safeTransferFrom(
            _hypercustos.rewardToken,
            msg.sender,
            _hypercustos.rewardPool,
            amount
        );

        // emit event
        emit HypercustosFunded(amount, duration);
    }
    ...
    function registerBonusToken(address bonusToken) external onlyOwner onlyOnline {
        ...
        require(
            _bonusTokenSet.length() < MAX_REWARD_TOKENS,
    ...
    function rescueTokensFromRewardPool(
        ...
        require(token != _hypercustos.rewardToken, "Hypercustos: invalid address");

        // check not attempting to wthdraw bonus token
        require(!_bonusTokenSet.contains(token), "Hypercustos: invalid address");

        // transfer tokens to recipient
        IRewardPool(_hypercustos.rewardPool).sendERC20(token, recipient, amount);
    }
    ...
    ///   - increase _hypercustos.totalStake
    ///   - increase _hypercustos.totalStakeUnits
    ///   - increase _hypercustos.lastUpdate
    ...
    function stake(
        ...
        require(isValidVault(vault), "Hypercustos: vault is not registered");

        // verify non-zero amount
        require(amount != 0, "Hypercustos: no amount staked");
        ...
        require(
            ...
            "Hypercustos: MAX_STAKES_PER_VAULT reached"
        );
        ...
        // update cached total vault and Hypercustos amounts
        vaultData.totalStake = vaultData.totalStake.add(amount);
        _hypercustos.totalStake = _hypercustos.totalStake.add(amount);

        // call lock on vault
        IUniversalVault(vault).lockERC20(_hypercustos.stakingToken, amount, permission);
    ...
    ///   - only online
    /// state scope:
    ///   - decrease _hypercustos.rewardSharesOutstanding
    ///   - decrease _hypercustos.totalStake
    ///   - increase _hypercustos.lastUpdate
    ///   - modify _hypercustos.totalStakeUnits
    ...
    function unstakeAndClaim(
        ...
        // verify non-zero amount
        require(amount != 0, "Hypercustos: no amount unstaked");

        // validate recipient
        _validateAddress(recipient);

        // check for sufficient vault stake amount
        require(vaultData.totalStake >= amount, "Hypercustos: insufficient vault stake");

        // check for sufficient Hypercustos stake amount
        // if this check fails, there is a bug in stake accounting
        assert(_hypercustos.totalStake >= amount);
        ...
        RewardOutput memory out =
            calculateRewardFromStakes(
                ...
                amount,
                unlockedRewards,
                _hypercustos.totalStakeUnits,
                block.timestamp,
                _hypercustos.rewardScaling
            );
        ...
        _hypercustos.totalStake = _hypercustos.totalStake.sub(amount);
        _hypercustos.totalStakeUnits = out.newTotalStakeUnits;

        // unlock staking tokens from vault
        IUniversalVault(vault).unlockERC20(_hypercustos.stakingToken, amount, permission);
        ...
        if (out.reward > 0) {
            ...
            uint256 sharesToBurn =
                _hypercustos.rewardSharesOutstanding.mul(out.reward).div(remainingRewards);

            // burn claimed shares
            _hypercustos.rewardSharesOutstanding = _hypercustos.rewardSharesOutstanding.sub(
                sharesToBurn
            );
            ...
            if (_bonusTokenSet.length() > 0) {
                for (uint256 index = 0; index < _bonusTokenSet.length(); index++) {
                    ...
                    uint256 bonusAmount =
                        IERC20(bonusToken).balanceOf(_hypercustos.rewardPool).mul(out.reward).div(
                            remainingRewards
                        );

                    // transfer bonus token
                    IRewardPool(_hypercustos.rewardPool).sendERC20(
                        bonusToken,
                        recipient,
                        bonusAmount
                    );
            ...
            // emit event
            emit RewardClaimed(vault, recipient, _hypercustos.rewardToken, out.reward);
        }
    }
    ...
    function rageQuit() external override {
        ...
        require(_vaultData.stakes.length != 0, "Hypercustos: no stake");
        ...
        _hypercustos.totalStake = _hypercustos.totalStake.sub(_vaultData.totalStake);
        _hypercustos.totalStakeUnits = _hypercustos.totalStakeUnits.sub(
    ...
    function _updateTotalStakeUnits() private {
        // update cached totalStakeUnits
        _hypercustos.totalStakeUnits = getCurrentTotalStakeUnits();
        // update cached lastUpdate
        _hypercustos.lastUpdate = block.timestamp;
    }

    function _validateAddress(address target) private view {
        // sanity check target for potential input errors
        require(isValidAddress(target), "Hypercustos: invalid address");
    }
