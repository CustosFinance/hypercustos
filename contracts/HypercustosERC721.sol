...
contract HypercustosERC721 is IHypercustosERC721, Powered {
    ...
    uint public VIP_LM_REWARD_MULTIPLIER_NUM = 6;
    uint public VIP_LM_REWARD_MULTIPLIER_DENOM = 2;
    uint public PRIMARY_LM_REWARD_MULTIPLIER_NUM = 5;
    uint public PRIMARY_LM_REWARD_MULTIPLIER_DENOM = 2;
    uint public GOLD_LM_REWARD_MULTIPLIER_NUM = 2;
    uint public GOLD_LM_REWARD_MULTIPLIER_DENOM = 1;
    uint public RARE_LM_REWARD_MULTIPLIER_NUM = 3;
    uint public RARE_LM_REWARD_MULTIPLIER_DENOM = 2;
    uint public STANDARD_LM_REWARD_MULTIPLIER_NUM = 1;
    uint public STANDARD_LM_REWARD_MULTIPLIER_DENOM = 1;
    ...
    function registerVaultFactory(address factory) external {
        _admin();
        // add factory to set
        require(_vaultFactorySet.add(factory), "HypercustosERC721: vault factory already registered");
    ...
    function removeVaultFactory(address factory) external {
        ...
        require(_vaultFactorySet.remove(factory), "HypercustosERC721: vault factory not registered");
    ...
    function registerBonusToken(address bonusToken) external {
        ...
        require(_bonusTokenSet.length() < MAX_BONUS_TOKENS, "HypercustosERC721: max bonus tokens reached ");
    ...
    function startLMRewardsToken(address token, uint256 amount, uint256 duration, string memory rewardCalcTemplateName) public {
        _admin();
    ...
    function _tierMultipliedReward(uint nftId, address nftFactory, uint reward) private view returns (uint multipliedReward) {
        ...
        if (tierHash == keccak256(abi.encodePacked(VIP))) {
            multipliedReward = reward.mul(VIP_LM_REWARD_MULTIPLIER_NUM).div(VIP_LM_REWARD_MULTIPLIER_DENOM);
        } else if (tierHash == keccak256(abi.encodePacked(PRIMARY))) {
            multipliedReward = reward.mul(PRIMARY_LM_REWARD_MULTIPLIER_NUM).div(PRIMARY_LM_REWARD_MULTIPLIER_DENOM);
            ...
            multipliedReward = reward.mul(RARE_LM_REWARD_MULTIPLIER_NUM).div(RARE_LM_REWARD_MULTIPLIER_DENOM);
        } else if (tierHash == keccak256(abi.encodePacked(STANDARD))) {
            multipliedReward = reward.mul(STANDARD_LM_REWARD_MULTIPLIER_NUM).div(STANDARD_LM_REWARD_MULTIPLIER_DENOM);
        }
    }
    ...
    function stakeERC721(
        ...
        require(vaultData.tokens.length() < MAX_TOKENS_STAKED_PER_VAULT, "HypercustosERC721: MAX_TOKENS_STAKED_PER_VAULT reached");
    ...
    function unstakeERC721AndClaimReward(
        ...
        // check for sufficient vault stake amount
        require(vaultData.tokenStake[token] >= 1, "HypercustosERC721: insufficient vault token stake");
